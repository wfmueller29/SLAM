...
#' @param xlow - lower bound on the x-axis. This parameter is optional, and if you skip it, it will recalculate based on data
#' @param xhigh - upper bound on the x-axis. This parameter is optional, and if you skip it, it will recalculate based on data
#' @param ylow - lower bound on the y-axis. This parameter is optional, and if you skip it, it will recalculate based on data
#' @param yhigh - upper bound on the y-axis. This parameter is optional, and if you skip it, it will recalculate based on data
#' @param color1 - palette you want to use. This is optional - if you don't set it, r will use default pallette
#' @param color2 - if you want a different color for the lines than the points, indicate it here, 
...
#' @param show_max - this asks if you want to put a line through local maximum of y. Default is FALSE
#' @param span - this refers to span for LOESS lines. Default is set to r defalut, which is 0.75
#' @return Plot
#' @export
...
longplot <- function(
  ...
  df <- base::subset(base::subset(data, eval(parse(text = y2))), eval(parse(text = y3)))
  ...
  p1 <- ggplot2::ggplot(subset(df, eval(parse(text = sub))), ggplot2::aes_string(x = x, y = y)) + 
    
    #dots
    ggplot2::geom_point(ggplot2::aes_string(fill = by), alpha=0.5, position = "jitter", size = 1, shape=21, stroke = 0.01) +
    ggplot2::scale_fill_manual(values = color1) +
    
    #trendlines
    ggplot2::geom_smooth(ggplot2::aes_string(group = by), linetype = "solid", color = "black", size = 1.8, span = span) +
    ggplot2::geom_smooth(ggplot2::aes_string(color = by), linetype = "solid", span = span) +
    ...
    ggplot2::scale_linetype_manual(labels = c("All"), name = "", values = c("solid", "dashed", "solid"), guide = FALSE) +
  ...
  if (show_max==TRUE) {
    ...
    p1 <- p1 + ggplot2::geom_vline(xintercept=exact_x_value_of_the_curve_maximum) + 
      ggplot2::annotate(x = exact_x_value_of_the_curve_maximum, y = 20, label = round(exact_x_value_of_the_curve_maximum), geom='label')
