...
histoplot <- function(
  ...
  # point_color is the point color - a little lighter than the colour of the bar
  ...
  if(points == TRUE) {
    if(point_color != 'black') {
      plotty <- ggplot2::ggplot(data, ggplot2::aes_string(x = x, y = y)) +
        ggplot2::stat_summary(fun = 'mean', geom = geom, col = 'grey4', ggplot2::aes_string(fill = color_by)) +
        ggplot2::facet_grid(~ eval(parse(text = by)), space = 'free_x', scales = 'free_x') +
        ggplot2::scale_fill_manual(values = bar_color) +
        ggplot2::geom_jitter(
          ggplot2::aes_string(x = x, y = y, col = color_by), shape = 5, size = 0.5, position = ggplot2::position_jitter(0.2), alpha = 0.4
        ) +
        ggplot2::scale_colour_manual(values = point_color) +
        ggplot2::geom_errorbar(ggplot2::aes(ymin = means - st_dev, ymax = means + st_dev), col = errorbar_color, size = 0.3, width = 0.17) +
      ...
      plotty <- ggplot2::ggplot(data, ggplot2::aes_string(x = x, y = y)) +
        ...
        ggplot2::scale_fill_manual(values = bar_color) +
        ggplot2::geom_jitter(
          ggplot2::aes_string(x = x, y = y), col = 'black', shape = 5, size = 0.5, position = ggplot2::position_jitter(0.2), alpha = 0.4
        ) +
        ggplot2::scale_colour_manual(values = point_color) +
        ggplot2::geom_errorbar(ggplot2::aes(ymin = means - st_dev, ymax = means + st_dev), col = errorbar_color, size = 0.3, width = 0.17) +
        ggplot2::ylab(paste(sub('(.)', '\\U\\1', y, perl=TRUE), '(g)')) +
